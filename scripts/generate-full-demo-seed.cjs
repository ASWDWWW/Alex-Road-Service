/**
 * Export full platform sample dataset (same shape as ARS.Demo.buildSeedState)
 * plus keep existing Stripe UAT invoices (INV-2026-0100+).
 * Writes scripts/_full-demo-seed.json for MCP/Firestore seeding.
 */
const fs = require('fs');
...
const customers = [
  ...
  { id: 'C200', name: 'Stripe Test Fleet', company: 'Stripe Test Fleet', phone: '(732) 555-0199', email: 'billing@stripetest.example', type: 'Fleet', trucks: 1, status: 'Active', notes: 'Stripe Checkout UAT customer.', spentAmount: 349.94, version: 1, createdAt: iso(1) },
];
...
const contactSubmissions = [
  { id: 'lead_001', name: 'Robert Walsh', company: 'Walsh Hauling', email: 'rwalsh@walshhaul.demo', phone: '(732) 555-0201', service: 'Emergency Roadside', truck: '2020 Volvo VNL', location: 'I-95 Exit 9, NJ', message: 'Need roadside assistance for overheating — stranded on shoulder.', status: 'New', timestamp: iso(0), source: 'website-contact-form', createdAt: iso(0) },
  { id: 'lead_002', name: 'Angela Ruiz', company: 'Ruiz Cartage', email: 'angela@ruizcartage.demo', phone: '(732) 555-0202', service: 'PM Service', truck: 'Fleet of 6 units', location: 'Keasbey, NJ', message: 'Looking for fleet PM pricing and schedule availability.', status: 'New', timestamp: iso(1), source: 'website-contact-form', createdAt: iso(1) },
  { id: 'lead_003', name: 'Frank DiMarco', company: 'DiMarco Express', email: 'fdimarco@dimarco.demo', phone: '(732) 555-0203', service: 'DOT Inspection', truck: '3 tractors', location: 'Perth Amboy, NJ', message: 'Need DOT inspections scheduled for March.', status: 'New', timestamp: iso(2), source: 'website-contact-form', createdAt: iso(2) },
  { id: 'lead_004', name: 'Nina Patel', company: 'Patel Logistics', email: 'npatel@patellog.demo', phone: '(732) 555-0204', service: 'Brake Service', truck: '2019 Kenworth T680', location: 'Edison, NJ', message: 'Grinding noise from steer axle — need estimate.', status: 'New', timestamp: iso(4), source: 'website-contact-form', createdAt: iso(4) },
  { id: 'lead_005', name: 'Howard Greene', company: 'Greene Waste Services', email: 'hgreene@greenewaste.demo', phone: '(732) 555-0205', service: 'Other', truck: 'Roll-off fleet', location: 'Woodbridge, NJ', message: 'Hydraulic cylinder leak on roll-off truck.', status: 'New', timestamp: iso(6), source: 'website-contact-form', createdAt: iso(6) },
];
...
const collections = {
  customers: customers.map((d) => ({ id: d.id, doc: toDoc(d) })),
  trucks: trucks.map((d) => ({ id: d.id, doc: toDoc(d) })),
  workOrders: workOrders.map((d) => ({ id: d.id, doc: toDoc(d) })),
  estimates: estimates.map((d) => ({ id: d.id, doc: toDoc(d) })),
  invoices: invoices.map((d) => ({ id: d.id, doc: toDoc(d) })),
  payments: payments.map((d) => ({ id: d.id, doc: toDoc(d) })),
  inventory: inventory.map((d) => ({ id: d.id, doc: toDoc(d) })),
  inventoryTransactions: inventoryTransactions.map((d) => ({ id: d.id, doc: toDoc(d) })),
  contact_submissions: contactSubmissions.map((d) => ({ id: d.id, doc: toDoc(d) })),
};
...
const outPath = path.join(__dirname, '_full-demo-seed.json');
fs.writeFileSync(outPath, JSON.stringify({ collections, settings }, null, 0));
